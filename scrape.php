...
(static function () {
    ...
    if (!Database::schema()->hasTable('therapists')) {
        Database::schema()->create('therapists', function (Blueprint $table) {
            $table->id();
            ...
            $table->string('offersOnlineTherapy')->nullable();
            $table->string('acceptingAppointments')->nullable();
    ...
    do{
        $response = $http->get($api, ['category' => 'african-american', 'page' => $page]);
        $crawler = new Crawler($response->body());
        $crawler->filter('div.results-row')->each(function (Crawler $node) {
